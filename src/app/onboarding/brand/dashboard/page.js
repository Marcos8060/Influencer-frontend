...
const Dashboard = () => {
  ...
  // Calculate stats
  const totalCampaigns = Array.isArray(brandCampaigns)
  ...
  const pendingApplicants = allCollaborators.filter(
    (c) => c.status === "pending"
  ).length;
  const totalProducts = Array.isArray(brandCampaigns)
    ? brandCampaigns.reduce(
        (acc, c) => acc + (c.products ? c.products.length : 0),
        0
      )
    : 0;
        ...
        <div className="grid md:grid-cols-2 grid-cols-1 gap-8 mt-8">
          {[...Array(2)].map((_, i) => (
            <Skeleton
              ...
              key={i}
              paragraph={{ rows: 8 }}
              style={{ borderRadius: 16, height: 320 }}
            />
          ))}
        </div>
        <div className="grid md:grid-cols-2 grid-cols-1 gap-8 mt-8">
